...
*Figure 6. aggregation levels in surveyed papers realtively to apapers with classical forest plots.*     
*Legend description (numbers of papers with at least one plot with given feature):*   
tax = papers with effect sizes aggregated to taxon level in the forest(-like) plots (N = `r length(papers_taxon)`)       
stu = papers with effect sizes aggregated to study level in the forest(-like) plots (N = `r length(papers_study)`)         
ind = papers with individual effect sizes presented in the forest(-like) plots (N = `r length(papers_individual)`)     
...
*Figure 7. Species-level aggregartion in surveyed papers.*    
*Legend description (numbers of papers with at least one plot with given feature):*   
...
### 10. What is the frequency of cattepillar-like plots with individual effect sizes plotted with CI?         

To answer this question, we re-evaluated a definition of cattepillar plot as:   
a) a classical forest plot (non-aggregated dot-type with CI) without overall mean (Kossmeier et al. 2020. Charting the landscape of graphical displays for meta-analysis and systematic reviews: a comprehensive review, taxonomy, and feature analysis BMC Med Res Methodol. 2020; 20: 26).    
b) individual ES are ordered by ES size, they have CI and they can heave or not have labels (Spiegelhalter D.J. 2005. Funnel plots for comparing institutional performance. Statistics in Medicine,24(8), 1185-1202).    
For these two definitions we created a list of papers containing at least one plot matching the given definition of a catepillar plot.
  
**Answer 10.**    
Papers presenting catepillar-like plots: 
a) There were 4 papers that were initially classified as potetially classcial forest plots but later excluded due to not showing the overall mean effect size in the same plot panel: ft037, ft084, ft109, ft181.
b) Re-evaluate papers: `r sort(papers_individual)`     
