...
We processed the coded data to create a paper-level table, with one consensus record per table, with information  that allowed us to answer the following questions [with predictions]:   
...
**Answer 3b.**  [MAKE TABLE WITH REFS!]
...
The remaining assessed papers were excluded due to not showing the overall mean effect size in the same plot panel.      
...
#### STAGE2 survey coding - results

During STAGE2, two researchers (ML, YY) assessed the extracted figures that were earlier classified as presenting forest plot-like aggregated effect estimates. They classified additional properties of these plots: whether overall meta-analytic mean was presented in the same plot (for each relevant figure panel). Thepliot coding was done on 10 randomly selected figures, then YY conducted main extraction of all figures, with all labelled as other/unsure at any point, checked and discussed with ML.

The questions to be answered by STAGE2 coding:   
...
PREDICTIONS: in EcoEvo – we have a mixture of forest-like plots (based meta-regressions, without overall mean) and summary forest plots (based on subgroup analyses, often with overall mean). The latter should be considered a part of the former.   
