---
title: "Supplementary Materials (Literature Survey) for - The Ecology and Evolution of Forest Plots and their Domestication: Introducing Orchard Plots"
author: "Shinichi Nakagawa, Malgorzata Lagisz, Rose O'Dea, Daniel Noble, Joanna Rutkowska, Alistair Senior"
...
The STAGE 1 survey was designed to answer the following questions:   
 ...
 4.	What are the frequences of aggregated data in the forest plot at the study, taxon, or other levels? How often non-aggregated data is presented?     
 ...
 We had the following predictions for the above questions:    
  ...
  3. We expected “dot” to be most popular, as this is the “classical/original” forest plot style; we expected dot scaling to be likely underutilised.   
...
From the list of the files ID's in the dataset we assessed how many papers presented some kind of forest or forest-like plot (see the Table below).
...
all_other = all other papers with forest(-like) plots (N = `r length(all)`)    
...
all_other = all papers with forest(-like) plots (N = `r length(all)`)    
...
*Legend description:*   
tax = papers with effect sizest aggregated to taxon level in the forest(-like) plots (N = `r length(papers_taxon)`)        
...
**Answer 5.**  
Out of 16 papers manually re-assessed, 12 contaibed  "original/classical" forest plots (see table below for the references).   
...
## Additional refinements of STAGE1 literature survey  

### 6. What is the frequency of forest-like plots with species as aggregation levels?    

To answer this question we manually re-evaluated the subset of `r length(papers_taxon)` papers which used some sort of taxonomic grouping for aggregating effect sizes. We then created a list of papers conatining at least one plot with specis-level aggregation of effect sizes.     

IDs of papers re-evaluated papers: `r sort(papers_taxon)`     

**Answer 6.** 
Final list of IDs papers presenting "species" forest-like plots (with relevant figure file ID):     
...
*Legend description:*   
species = papers with effect sizest aggregated to species level in the forest(-like) plots (N = `r length(papers_species)`)       
...
We re-assessed forest-like plots with "aggregated effect sizes" to record how often overall mean is presented alongside aggregated effect sizes and how aggregeated effect sizes are calculated.     
...
Predictions: we expected to find a mixture of forest-like plots (based meta-regressions, without overall mean) and summary forest plots (based on subgroup analyses, often with overall mean) in ecological and evolutionary meta-analyses. Summary forest plosts could be considered a special case of the forest-like plots.   
...
In stage2 we re-assessed and code `r length(unique(S2_data$paperID))` papers. 


```{r load coded data STAGE2, eval = TRUE}
# getting the data and formating some variables (turning chraracter vectors to factors)
S2_data <- read_excel("./data/Data collection for the _orchard plot_ manuscript - STAGE2 (Responses)_cleaned.xlsx", na = "NA") %>% 
   mutate_if(is.character, as.factor)
...
#(dim(papers_with_overall_mean)[1] + dim(papers_without_overall_mean)[1] - length(unique(S2_data1$paperID)))/2 # 9 papers with both plots with and without overall mean
```

**Answer 8.**	   
Overall mean is not commonly presented alongsiet aggregated point estimates in forest-like plots - only `r sum_overall_mean[2,2]` (`r round(sum_overall_mean[2,3], 2)` %) out of `r sum(sum_overall_mean$sum)` aggeregated forest-like plots assessed.

References of papers with aggregated forest-like plots with overall mean:    
```{r table of papers with overall mean, eval = TRUE}
# making a scrollable table
dat8table <- papers_data %>% filter(fulltext_ID %in% papers_with_overall_mean) %>% select(fulltext_ID, authors, pub_year, title, journal, volume, pages)
...
References of papers with aggregated forest-like plots without overall mean:    
```{r table of papers without overall mean, eval = TRUE}
# making a scrollable table
dat8table <- papers_data %>% filter(fulltext_ID %in% papers_without_overall_mean) %>% select(fulltext_ID, authors, pub_year, title, journal, volume, pages)
...
#(dim(papers_with_metaregression)[1] + dim(papers_with_metaanalysis)[1] - length(unique(S2_data2$paperID)))/2 # 9 papers with both plots with meta-analysis and meta-regression
```

**Answer 9.**	   
Most commonly, forest-like plots in ecology and evolution are based on meta-regression models -  `r sum_aggr_estimate_by[2,2]` (`r round(sum_aggr_estimate_by[2,3], 2)` %) out of `r sum(sum_aggr_estimate_by$sum)` aggeregated forest-like plots assessed.    

References of papers with aggregated forest-like plots based on meta-regression models:    
```{r table of papers with meta-regression, eval = TRUE}
# making a scrollable table
dat9table <- papers_data %>% filter(fulltext_ID %in% papers_with_metaregression) %>% select(fulltext_ID, authors, pub_year, title, journal, volume, pages)
...
References of papers with aggregated forest-like plots based on meta-regression models:    
```{r table of papers with meta-analysis, eval = TRUE}
# making a scrollable table
dat9table <- papers_data %>% filter(fulltext_ID %in% papers_with_metaanalysis) %>% select(fulltext_ID, authors, pub_year, title, journal, volume, pages)
